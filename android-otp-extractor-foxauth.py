...
# Encode a message for Firefox extension transmission, given its content.
...
# Send an encoded message to stdout for Firefox extension.
...
if __name__ == '__main__':
    ...
    parser.add_argument('--firefox-extension', action='store_true', help='output binary data to a firefox extension')
    ...
    accounts = set()  # type: Set[OTPAccount]
    ...
    if args.firefox_extension:
        ...
        for account in accounts:
            data = fallback.copy()
            data['localAccountName'] = account.name
            data['localSecretToken'] = account.secret
            if account.issuer:
                data['localIssuer'] = account.issuer
            if isinstance(account, HOTPAccount):
                data['localOTPType'] = 'HMAC based'
                if account.algorithm == 'SHA1':
                    data['localOTPAlgorithm'] = "SHA-1"
                else:
                    data['localOTPAlgorithm'] = account.algorithm
                data['localOTPDigits'] = str(account.digits)
            elif isinstance(account, TOTPAccount):
                data['localOTPType'] = 'Time based'
                if account.algorithm == 'SHA1':
                    data['localOTPAlgorithm'] = "SHA-1"
                else:
                    data['localOTPAlgorithm'] = account.algorithm
                data['localOTPPeriod'] = str(account.period)
                data['localOTPDigits'] = str(account.digits)
            result.append(data)
